...
{TYPES}
    ...
    public static function fromArray(array $firstRow, array $records, $sheetName = null)
    {
        $firstRowBold = PhpXlsxGenerator::boldItems($firstRow);
        $rows = [];
        $rows[] = $firstRowBold;
...
<sheetView workbookViewId="0"' . ($this->rtl ? ' rightToLeft="1"' : '');
    ...
    protected static function boldItems(array $records): array
    {
        $returnArr = [];
        foreach ($records as $record) {
            $returnArr[] = PhpXlsxGenerator::bold($record);
        }
